...
install:
ifneq (,$(wildcard /lib/modules/$(KVER)/$(MDIR)/ax88179_178a.ko))
	gzip /lib/modules/$(KVER)/$(MDIR)/ax88179_178a.ko
endif
	make -C $(KDIR) M=$(PWD) INSTALL_MOD_DIR=$(MDIR) modules_install
	depmod -a

uninstall:
ifneq (,$(wildcard /lib/modules/$(KVER)/$(MDIR)/$(TARGET).ko))
	rm -f /lib/modules/$(KVER)/$(MDIR)/$(TARGET).ko
endif
	depmod -a
